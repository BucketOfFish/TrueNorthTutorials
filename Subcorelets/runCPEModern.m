...
subcorelets = subcorelets(); % make corelet
subcorelets.inputs(1).setExternalInput('simpleInput'); % recognize and give a name to the input connector
subcorelets.outputs(1).setExternalOutput('simpleOutput'); % recognize and give a name to the output connector

subcorelets.cr_disp; % print info via dispThis()
verified = subcorelets.verify(0); % verify via verifyThis() - 0 to suppress output

%%%%%%%%%%%%%%
% Save model %
%%%%%%%%%%%%%%

modelName = 'subcorelets'; % base name for the model files
outputFolder = 'Test'; % where the files will be saved

model = makeModel(subcorelets, modelName, outputFolder); % create all model files
