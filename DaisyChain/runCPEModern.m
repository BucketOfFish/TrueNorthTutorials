...
daisyChain = daisyChain(5); % make corelet
daisyChain.inputs(1).setExternalInput('simpleInput'); % recognize and give a name to the input connector
daisyChain.outputs(1).setExternalOutput('simpleOutput'); % recognize and give a name to the output connector

daisyChain.cr_disp; % print info via dispThis()
verified = daisyChain.verify(0); % verify via verifyThis() - 0 to suppress output

%%%%%%%%%%%%%%
% Save model %
%%%%%%%%%%%%%%

modelName = 'daisyChain'; % base name for the model files
outputFolder = 'Test'; % where the files will be saved

model = makeModel(daisyChain, modelName, outputFolder); % create all model files
